 ...
 */
  ...
  class CaptchaComponent extends Component	{
    ...
    function __imageCaptcha() {
    	$fonts = array('AllerDisplay','Aller_Bd','Aller_BdIt','Aller_It','Aller_Lt','Aller_LtIt','Aller_Rg');
    	//$font_path = dirname(dirname(dirname(__FILE__))) . DS . 'Lib' . DS . 'Fonts';
    	$font_path = $_SERVER['DOCUMENT_ROOT'] . $this->request->webroot . "webroot/fonts";
    	$font_name = $fonts[array_rand($fonts)] . ".ttf";
    	$set_font = $font_path . DS . $font_name;
        ...
        //$text_color = imagecolorallocate($image, 38, 15, 15);
        ...
        if($this->__TTFEnabled())  {
            ...
            $textbox = imagettfbbox($font_size, $angle, $set_font, $code) or die('Error in imagettfbbox function');
           
            $x = ($width - $textbox[4])/2;
            $y = ($height - $textbox[5])/2;
            $y -= 5;
            imagettftext($image, $font_size, $angle, $x, $y, $text_color,$set_font, $code) or die('Error in imagettftext function');
